...
const getMyLevelIncomes = async (req, res) => {
    try {
        ...
        // Group by from_purchase_id + level so each LevelIncome row only sees its own slice
        ...
        // Key = "<purchaseId>_<level>"
        const releasedMap = {};
        distributions.forEach(dist => {
            const key = `${String(dist.from_purchase_id)}_${dist.level}`;
            releasedMap[key] = (releasedMap[key] || 0) + dist.monthly_amount;
        });

        // 5b. Fetch products to get the approved date (update_at)
        const productIds = [...new Set(incomes.map(inc => inc.product_id).filter(id => id))];
        const Product = require('../models/Product');
        const products = await Product.find({
            $or: [
                { _id: { $in: productIds.filter(id => require('mongoose').Types.ObjectId.isValid(id)) } },
                { id: { $in: productIds } }
            ]
        }).lean();
        ...
        const incomesWithDetails = incomes.map(inc => {
            ...
            // Match by same product AND same level so released never exceeds total for that level
            const key = `${String(inc.product_id)}_${inc.level}`;
            const releasedAmount = releasedMap[key] || 0;
            
            const product = inc.product_id ? productMap[String(inc.product_id)] : null;
            // The product approved date is product.update_at if product is approved (or fallback to product.cereate_at/create_at)
            const approvedDate = (product && (product.approve == 1 || product.approve == '1'))
                ? (product.update_at || product.cereate_at || product.create_at || inc.create_at || inc.created_at)
                : (inc.create_at || inc.created_at);
...
const getDashboardStats = async (req, res) => {
    try {
        ...
        // Fetch all monthly distributions
        ...
        const totalPending = distributions.reduce((sum, dist) => {
            if (dist.scheduled_date > now) return sum + dist.monthly_amount;
            return sum;
        }, 0);

...
const getAllBankRequests = async (req, res) => {
    try {
        ...
        // Filter by date if provided (using ObjectId creation timestamp)
        if (startDate || endDate) {
            ...
            if (startDate) {
                ...
                dateQuery.$gte = mongoose.Types.ObjectId.createFromTime(startSeconds);
            }
            if (endDate) {
                const end = new Date(endDate);
                end.setHours(23, 59, 59, 999);
                const endSeconds = Math.floor(end.getTime() / 1000);
                dateQuery.$lte = mongoose.Types.ObjectId.createFromTime(endSeconds);
            }
            query._id = dateQuery;
        }

...
const addOrUpdateWallet = async (req, res) => {
    const wallet_add = typeof req.body.wallet_add === 'string' ? req.body.wallet_add.trim() : '';
    const userIds = getWalletIdentifiers(req.user);
    const canonicalUserId = req.user._id.toString();
    ...
    try {
        ...
        let userWallet = await Wallet.findOne({ user_id: { $in: userIds } }).sort({ updatedAt: -1 });

        if (userWallet) {
            if (normalizeWalletAddress(userWallet.wallet_add) !== normalizeWalletAddress(wallet_add)) {
            ...
            userWallet = await Wallet.create({
                ...
                wallet_add,
                approve: 1 // Approved (Auto approved)
            });
        }
...
const getMyWallet = async (req, res) => {
    try {
        const wallet = await Wallet.findOne({
            user_id: { $in: getWalletIdentifiers(req.user) }
        }).sort({ updatedAt: -1 });
...
const getAllWallets = async (req, res) => {
    try {
        ...
        const users = await User.find({}).select('_id user_id id full_name email referral_id').lean();
...
const getWalletByUser = async (req, res) => {
    try {
        ...
        const identifiers = user ? getWalletIdentifiers(user) : [req.params.id];
        const wallet = await Wallet.findOne({ user_id: { $in: identifiers } }).sort({ updatedAt: -1 });
        res.status(200).json(wallet || {});
    } catch (error) {
        res.status(500).json({ message: error.message });
    }

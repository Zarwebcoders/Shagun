...
const createMiningBonus = async (req, res) => {
    ...
    try {
        ...
        // Create notification for mining bonus
        try {
            await Notification.create({
                user_id,
                message: `You received a Mining Bonus of ₹${Number(amount).toFixed(2)}.`,
                type: 'income',
                path: '/mining-history'
            });
        } catch (err) {
            console.error("Failed to create mining bonus notification:", err);
        }

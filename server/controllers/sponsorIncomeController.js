...
const getMyIncome = async (req, res) => {
    try {
        ...
        const queryIds = [req.user._id.toString(), req.user.id, req.user.user_id].filter(id => id);
        const income = await SponsorIncome.find({ user_id: { $in: queryIds } })
            .populate('from_user_id', 'full_name email referral_id')
            .sort({ created_at: -1 });

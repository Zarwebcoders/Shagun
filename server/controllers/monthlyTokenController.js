        ...
        for (const dist of pendingDistributions) {
            try {
                ...
                if (user) {
                    ...
                    // Create notification for monthly token release
                    try {
                        const Notification = require('../models/Notification');
                        await Notification.create({
                            user_id: user._id,
                            message: `A monthly payout of ₹${dist.monthly_amount.toFixed(2)} has been unlocked and is now withdrawable.`,
                            type: 'income',
                            path: '/level-income'
                        });
                    } catch (err) {
                        console.error("Failed to create monthly payout release notification:", err);
                    }

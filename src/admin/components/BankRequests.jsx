...
export default function BankRequests() {
    ...
    const filteredAccounts = accounts.filter(acc =>
        ...
        (acc.user_details?.full_name?.toLowerCase() || "").includes(searchTerm.toLowerCase()) ||
        (acc.user_details?.email?.toLowerCase() || "").includes(searchTerm.toLowerCase()) ||
        (acc.acc_num || "").includes(searchTerm)
    );
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-3">
                    <div className="relative">
                        ...
                        <input
                            type="text"
                            placeholder="Search Name/Email/Acc No..."
                            value={searchTerm}
                                    ...
                                    <tr key={acc._id} className="hover:bg-white/5 transition-colors">
                                        <td className="p-4 text-sm text-gray-300">
                                            <div className="flex flex-col">
                                                <span className="text-white font-medium">{acc.user_details?.full_name || 'Unknown User'}</span>
                                                <span className="text-xs text-gray-500">{acc.user_details?.email || acc.user_id}</span>
                                            </div>
                                        </td>

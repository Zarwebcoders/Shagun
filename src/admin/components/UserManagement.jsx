...
export default function UserManagement() {
        ...
        const fetchAllPackages = async () => {
            try {
                ...
                if (data.packageTypes) {
                    setAvailablePackages(data.packageTypes);
                } else if (Array.isArray(data)) {
                    const pkgs = [...new Set(data.map(p => p.packag_type).filter(Boolean))];
                    setAvailablePackages(pkgs);
                }
            } catch (err) {
                console.error("Error fetching packages for filter:", err);
            }
        };
        fetchAllPackages();
    }, []);
    ...
    const handleImpersonateUser = async (user) => {
        if (window.confirm(`Are you sure you want to login as ${user.full_name}?`)) {
            try {
                const { data } = await client.post(`/users/${user._id}/impersonate`);
                
                const encodedData = encodeURIComponent(JSON.stringify(data));
                
                // Open user dashboard in a new tab with impersonation parameter
                window.open(`/dashboard?impersonate=${encodedData}`, '_blank');
                
                toast.success(`Opening dashboard for ${user.full_name} in a new tab`);
            } catch (error) {
            ...
            <div className="bg-[#0f0f1a] rounded-xl border border-teal-500/20 p-5 shadow-xl space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
                    ...
                    <div className="space-y-1.5">
                        <label className="text-xs font-medium text-gray-400 uppercase tracking-wider">Approved Package</label>
                        <select
                            value={packageFilter}
                                    ...
                                    <tr key={user._id} className="hover:bg-[#1a1a2e] transition-colors">
                                        ...
                                        <td className="px-6 py-4 text-sm">
                                            <div className="flex flex-col items-start gap-1">
                                                <span className="text-teal-400 text-xs bg-teal-500/10 px-1.5 py-0.5 rounded font-medium">Ref: {user.referral_id || 'N/A'}</span>
                                                <span className="text-purple-400 text-xs bg-purple-500/10 px-1.5 py-0.5 rounded font-medium">Spon: {user.sponsor_id || 'N/A'}</span>
                                            </div>
                                        </td>
                                        ...
                                        <td className="px-6 py-4 text-sm">
                                            {user.wallet_address ? (
                                                <span
                                                    className="text-teal-300 font-mono text-[10px] bg-teal-500/5 border border-teal-500/20 px-1.5 py-0.5 rounded cursor-pointer hover:bg-teal-500/20 transition-colors"
                                                    title={user.wallet_address}
                                                    onClick={() => { navigator.clipboard.writeText(user.wallet_address); toast.success('Wallet copied!'); }}
                                                >
                                        ...
                                        <td className="px-6 py-4 text-sm">
                                            <div className="flex flex-col items-center gap-1">
                                                {user.approved_packages && user.approved_packages.length > 0 ? (
                                                    user.approved_packages.map((pkg, idx) => (
                                                        <span key={idx} className="text-green-400 text-[10px] bg-green-500/10 px-1.5 py-0.5 rounded font-bold border border-green-500/20 uppercase tracking-wider block text-center min-w-[80px]">
                                                            {typeof pkg === 'object' ? `${pkg.name} (${pkg.quantity})` : pkg}
                                                        </span>
                                                    ))
                                                ) : (
                                        ...
                                        <td className="px-6 py-4 text-sm">
                                             <div className="flex items-center justify-center gap-1.5">
                                                 <span className="text-gray-300 font-mono text-xs bg-[#1a1a2e] px-2 py-1 rounded border border-teal-500/20 min-w-[70px] inline-block text-center">
                    ...
                    <div className="fixed inset-0 bg-black/70 flex items-center justify-center p-4 z-50 animate-fadeIn">
                        <div className="bg-[#0f0f1a] rounded-xl p-6 max-w-4xl w-full border border-teal-500/30 max-h-[90vh] overflow-y-auto">
                            ...
                            <div className="space-y-6">
                                ...
                                <div className="flex items-center gap-4 pb-6 border-b border-teal-500/30">
                                    ...
                                    <div>
                                        ...
                                        <div className="flex gap-2 mt-1">
                                            <p className="text-teal-500 text-sm bg-teal-500/10 px-2 py-0.5 rounded font-medium">Referral ID: {selectedUser.referral_id || 'N/A'}</p>
                                            <p className="text-purple-500 text-sm bg-purple-500/10 px-2 py-0.5 rounded font-medium">Sponsor ID: {selectedUser.sponsor_id || 'N/A'}</p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                {/* Approved Packages */}
                                <div>
                                    <h5 className="text-lg font-bold text-white mb-3">Approved Packages</h5>
                                    <div className="flex flex-wrap gap-2">
                                        {selectedUser.approved_packages && selectedUser.approved_packages.length > 0 ? (
                                            selectedUser.approved_packages.map((pkg, idx) => (
                                                <span key={idx} className="text-green-400 text-xs bg-green-500/10 px-2.5 py-1 rounded-full font-bold border border-green-500/20 uppercase tracking-wider">
                                                    {typeof pkg === 'object' ? `${pkg.name} (${pkg.quantity})` : pkg}
                                                </span>
                                            ))
                                        ) : (
                                            <p className="text-gray-500 text-sm">No approved packages purchased yet.</p>
                                        )}
                                    </div>
                                </div>

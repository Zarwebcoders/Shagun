...
export default function VendorWithdrawalRequests() {
    ...
    const fetchRequests = async () => {
        try {
            const { data } = await client.get('/api/vendor-withdraw/all');
            setRequests(data);
            setLoading(false);
        } catch (error) {
    ...
    const handleStatusUpdate = async (id, status) => { // status: 1 for Approve, 0 for Reject
        try {
            await client.put(`/api/vendor-withdraw/${id}`, { approve: status });
            toast.success(status === 1 ? "Request Approved" : "Request Rejected");
                                ...
                                filteredRequests.map((req) => (
                                    <tr key={req._id} className="hover:bg-[#1a1a2e]/50 transition-colors">
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <span className="text-white font-medium font-mono">{req.vendor_id}</span>
                                        </td>
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <span className="text-purple-400 font-bold">₹{req.amount.toLocaleString()}</span>
                                        </td>
                                        <td className="px-6 py-4 whitespace-nowrap">

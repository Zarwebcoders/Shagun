...
export default function TransactionMonitor({ defaultType = "all" }) {
    ...
    const filteredTransactions = transactions.filter(tx => {
        // Search term (Matches full name, email, or description)
        ...
        const matchesType = selectedType === "all" ? true : (
            selectedType === "referral_income" 
                ? (tx.type === "referral_income" || tx.type === "referral")
                : tx.type === selectedType
        );

        // ID filter: checks user_id, referral_id, or database _id of the user
        const matchesId = idFilter ? (
            (tx.user?.user_id?.toLowerCase() || "").includes(idFilter.toLowerCase()) ||
            (tx.user?.referral_id?.toLowerCase() || "").includes(idFilter.toLowerCase()) ||
            (tx.user?._id?.toLowerCase() || "").includes(idFilter.toLowerCase()) ||
            (tx.relatedUser?.user_id?.toLowerCase() || "").includes(idFilter.toLowerCase()) ||
            (tx.relatedUser?.referral_id?.toLowerCase() || "").includes(idFilter.toLowerCase())
        ) : true;
        ...
        return matchesSearch && matchesType && matchesId && matchesDate;
    });

    // Compute stats dynamically from filtered transactions
    const totalFilteredAmount = filteredTransactions.reduce((acc, tx) => acc + (tx.amount || 0), 0);
    
    // Referral income stats (filtered & overall)

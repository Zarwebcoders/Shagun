                ...
                <div className="bg-[#0f0f1a] p-4 rounded-xl border border-teal-500/20 shadow-xl">
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 items-end">
                        ...
                        <div className="space-y-1.5">
                            <label className="text-xs font-medium text-gray-400 uppercase tracking-wider block">Search</label>
                            <div className="bg-[#1a1a2e] px-3 py-2 rounded-lg border border-teal-500/20 w-full">
                                <input
                                    type="text"
                                    placeholder="Search by name, ID, bank, account..."
                                    value={search}
                                    onChange={(e) => setSearch(e.target.value)}
                                    className="w-full bg-transparent border-none text-white focus:ring-0 text-sm outline-none"
                                />
                            </div>
                        </div>
                        ...
                        <div className="space-y-1.5 lg:col-span-2">
                            <label className="text-xs font-medium text-gray-400 uppercase tracking-wider block">Submission Date Range</label>
                    ...
                    <table className="w-full">
                        <thead className="bg-[#1a1a2e] border-b border-teal-500/20">
                            <tr>
                                ...
                                <th className="px-6 py-4 text-left text-xs font-semibold text-gray-400 uppercase tracking-wider">Submission Date</th>
                                <th className="px-6 py-4 text-right text-xs font-semibold text-gray-400 uppercase tracking-wider">Actions</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-teal-500/10">
                            {loading ? (
                                <tr><td colSpan="7" className="px-6 py-8 text-center text-gray-500">Loading accounts...</td></tr>
                            ) : accounts.length === 0 ? (
                                <tr><td colSpan="7" className="px-6 py-8 text-center text-gray-500">No accounts found</td></tr>
                            ) : (
                                accounts.map((acc) => {
                                    const getCreateDate = (acc) => {
                                        if (acc.createdAt) return new Date(acc.createdAt);
                                        if (acc._id && acc._id.length === 24) {
                                            const timestamp = parseInt(acc._id.substring(0, 8), 16) * 1000;
                                            return new Date(timestamp);
                                        }
                                        return null;
                                    };
                                    const createDate = getCreateDate(acc);
                                    return (

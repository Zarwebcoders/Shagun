                            ...
                            >
                                ...
                                <td className="px-6 py-4 whitespace-nowrap">
                                    <div className="flex items-center gap-2 text-gray-300 font-mono text-xs">
                                        ...
                                        {record.wallet_address && record.wallet_address !== 'N/A' && record.wallet_address.length > 16 ? 
                                            `${record.wallet_address.substring(0, 6)}...${record.wallet_address.substring(record.wallet_address.length - 6)}` 
                                ...
                                <td className="px-6 py-4 whitespace-nowrap font-bold">
                                    {record.status === 'LAPSED' ? (
                                        <span className="text-rose-400/80">{Number(record.amount || 0).toFixed(4)} SGN</span>
                                    ) : Number(record.amount || 0) > 0 ? (
                                        <span className="text-teal-400">+{Number(record.amount).toFixed(4)} SGN</span>
                                    ) : (
                                        <span className="text-gray-500 text-xs font-normal italic">Pending setup</span>
                                    )}
                                </td>
                                <td className="px-6 py-4 whitespace-nowrap">
                                ...
                                <td className="px-6 py-4 whitespace-nowrap text-right">
                                    {record.status === 'LAPSED' ? (
                                        record.hash && record.hash !== 'lapsed-on-chain' ? (
                                            <a
                                                href={`https://polygonscan.com/tx/${record.hash}`}
                                                target="_blank"
                                                rel="noopener noreferrer"
                                                className="px-2 py-1 rounded-full bg-rose-500/10 text-rose-500 hover:bg-rose-500/20 text-[10px] font-bold border border-rose-500/20 inline-flex items-center gap-1 transition-all cursor-pointer"
                                            ...
                                            <span className="px-2 py-1 rounded-full bg-rose-500/10 text-rose-500 text-[10px] font-bold border border-rose-500/20">
                                                LAPSED
                                            </span>
                                        )
                                    ) : record.hash && record.hash !== 'lapsed-on-chain' ? (
                                        <a
                                            ...
                                            target="_blank"
                                            rel="noopener noreferrer"
                                            className="px-2 py-1 rounded-full bg-green-500/10 text-green-500 hover:bg-green-500/20 text-[10px] font-bold border border-green-500/20 inline-flex items-center gap-1 transition-all cursor-pointer"
                                            title="View on Polygonscan"
                                        >
                                            SUCCESS
                                            <ChevronRight className="w-3 h-3" />

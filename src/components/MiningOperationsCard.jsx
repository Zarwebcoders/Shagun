...
export default function MiningOperationsCard({ status, miningPower, earningsToday, lastMinedAt, monthlyCount, onMine, totalMiningCount = 0, stakedBalance = 0, loading = false, isConnected = false }) {
    ...
    return (
        <div className="bg-[#1a1a2e]/40 backdrop-blur-xl border border-teal-500/20 rounded-3xl p-6 h-full">
            ...
            <div className="flex flex-col gap-4 mb-6">
                ...
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                    <button
                        onClick={onMine}
                        disabled={loading}
                        className="w-full sm:w-auto px-6 py-3 sm:py-2 rounded-xl border border-teal-500/50 font-bold transition-all duration-300 flex items-center justify-center gap-2 text-sm shadow-lg bg-gradient-brand text-white hover:shadow-teal-500/40 hover:-translate-y-0.5 active:translate-y-0 disabled:opacity-50 disabled:cursor-not-allowed relative isolate touch-manipulation"
                    >
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
                <div className="bg-[#0f0f1a]/60 p-4 rounded-2xl border border-white/5">
                    <span className="text-gray-400 text-xs uppercase tracking-wider">Pending Cycles</span>
                    <div className="text-2xl font-bold text-amber-400 mt-1">{24 - currentCycle} Cycles</div>
                    <p className="text-[10px] text-amber-500 mt-1">Out of 24 cycles total</p>
                </div>

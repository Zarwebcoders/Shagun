    ...
    useEffect(() => {
        const fetchUserData = async () => {
            try {
                ...
                const calculatedSponsorIncome = Number(summaryData.calculatedSponsorIncome || 0);
                ...
                const phaseDisplay = settingsData.currentPhase !== undefined && settingsData.currentPhase !== null
                    ? (typeof settingsData.currentPhase === 'number' || !isNaN(settingsData.currentPhase)
                        ? `Phase ${settingsData.currentPhase}`
                        : settingsData.currentPhase)
                    : "Phase 7";
                ...
                // Direct team size from summary response
                const directTeamCount = Number(summaryData.directTeamCount || 0);
                ...
                // Condition: If has approved product OR has airdrop token > 0 OR activates from admin
                const isAdminActive = String(userData.is_deleted) === "0";
                const hasAirdrop = Number(userData.airdrop_tokons || 0) > 0;
                const hasApprovedProduct = !!summaryData.hasApprovedProduct;
        ...
        >
            ...
            <motion.section variants={itemVariants} className="relative">
                <div className="bg-gradient-to-r from-teal-500/20 via-[#1a1a2e] to-[#040408] rounded-3xl p-6 md:p-8 border border-teal-500/30 overflow-hidden relative">
                    {/* Abstract Shapes */}
                    <div style={{ pointerEvents: 'none', width: 0, height: 0, overflow: 'hidden' }} className="absolute top-0 right-0 w-96 h-96 bg-teal-500/10 rounded-full blur-[100px] translate-x-1/3 -translate-y-1/3 pointer-events-none"></div>
                    <div style={{ pointerEvents: 'none', width: 0, height: 0, overflow: 'hidden' }} className="absolute bottom-0 left-0 w-64 h-64 bg-purple-500/10 rounded-full blur-[80px] -translate-x-1/3 translate-y-1/3 pointer-events-none"></div>
                    ...
                    <div className="flex flex-col lg:flex-row justify-between items-center gap-8 relative z-10">
                        <div className="flex-1 space-y-4">
                            ...
                            {walletError && (
                                <p className="text-sm text-rose-400 max-w-xl">
                                    {walletError}
                                </p>
                            )}
                        </div>
            ...
            {/* 12-Month Holding Rewards Section (Show after wallet connect) */}
            {isConnected && (
                <motion.section
                ...
                >
                    <div className="flex items-center gap-3 mb-6">
                        ...
                        <h3 className="text-2xl font-bold text-white">12-Month SGN Mining Rewards</h3>
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                        <StatsCard
                            title="Total SGN Reward (Total in 12 months)"
                            amount={`SGN ${(parseFloat(stakedBalance || "0") * 0.2).toFixed(2)}`}
                            color="#a855f7"
                            icon={BriefcaseIcon}
                        />
                        <StatsCard
                            title="Total Received Till Now"
                            amount={`SGN ${(parseFloat(stakedBalance || "0") * 0.05 * (holdingRewardsClaimed || 0)).toFixed(2)}`}
                            color="#2DD4BF"
                            icon={CheckCircleIcon}
                        />
                        <StatsCard
                            title="Available Balance"
                            amount={`SGN ${(parseFloat(stakedBalance || "0") * 0.2 - parseFloat(stakedBalance || "0") * 0.05 * (holdingRewardsClaimed || 0)).toFixed(2)}`}
                            color="#3B82F6"
                            icon={WalletIcon}
                        />
                    </div>
                    ...
                    <div className="bg-gradient-to-br from-[#1a1a2e] to-[#0f0f1a] rounded-3xl p-6 border border-teal-500/20 flex flex-col h-[calc(100%-3rem)]">
                        ...
                        <div className="bg-teal-500/10 rounded-2xl p-4 border border-teal-500/30 mb-6 text-center relative group">
                            ...
                            <button
                                ...
                                onClick={() => {
                                    const referralLink = `${window.location.origin}/signup?ref=${referralProgram.referralId}`;

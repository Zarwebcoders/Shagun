...
export default function KYC() {
        ...
        const fetchKYCStatus = async () => {
            try {
                ...
                if (res.data) {
                    ...
                    // If rejected, pre-populate the form
                    if (res.data.approval === 0) {
                        setFormData({
                            aadharNumber: res.data.aadhar || "",
                            aadharFrontFile: null,
                            aadharBackFile: null,
                            ...
                            profilePhoto: null,
                            agreementPhoto: null,
                            accountName: res.data.acc_name || "",
                            bankName: res.data.bank_name || "",
                            accountNumber: res.data.acc_num || "",
                            branch: res.data.branch || "",
                            ifscCode: res.data.ifsc_code || "",
                            chequePassbookFile: null,
                        });
                        setPreviewUrls({
                            aadharFront: res.data.aadharcard || null,
                            aadharBack: res.data.aadhar_back || null,
                            pan: res.data.pancard || null,
                            profile: res.data.profile_photo || null,
                            agreement: res.data.agreement || null,
                            chequePassbook: res.data.cheque_passbook || null,
                        });
                    }
                }
            } catch (error) {
    ...
    const handleFileChange = async (e, field) => {
        ...
        if (file) {
            ...
            const previewFieldMap = {
                ...
                'chequePassbookFile': 'chequePassbook'
            }
            ...
            if (file.type.startsWith('image/')) {
                try {
                    ...
                    setPreviewUrls((prev) => ({
                        ...prev,
                        [previewField]: compressedBase64
                    }));
                } catch (compressErr) {
                    console.error("Image compression failed, using fallback reader:", compressErr);
                    const reader = new FileReader()
                    reader.onloadend = () => {
                        setPreviewUrls((prev) => ({
                            ...prev,
        ...
        try {
            await client.post('/kyc', {
                ...
                documents: {
                    ...
                    chequePassbook: previewUrls.chequePassbook
                }
            });
    ...
    const removeFile = (field) => {
        const formDataFieldMap = {
            ...
            'chequePassbook': 'chequePassbookFile'
        }
                        ...
                        <div className="bg-red-500/10 border border-red-500/50 p-6 rounded-2xl mb-8 flex items-start gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-gray-400">
                                    Your previous submission was rejected. Please review and correct your details in the form below, then resubmit.
                                </p>
                            </div>
                        </div>
                    )}
                    ...
                    {step === 4 && (
                        ...
                        >
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div className="md:col-span-2">
                                    <UploadBox
                                        label="Cheque Book / Passbook Photo"
                                        file={formData.chequePassbookFile}
                                        preview={previewUrls.chequePassbook}
                                        onChange={(e) => handleFileChange(e, 'chequePassbookFile')}
                                        onRemove={() => removeFile('chequePassbook')}
                                    />
                                </div>
                            </div>
                        </motion.div>
        ...
        >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {[
                    ...
                    { label: 'Cheque Book / Passbook', url: kyc.cheque_passbook }
                ].map((doc, idx) => (
                    doc.url && (
        ...
        <div>
            ...
            {preview ? (
                <div className="relative rounded-xl overflow-hidden border border-teal-500 group h-48 w-full">
                    ...
                    <div className="absolute inset-0 hidden md:block bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none" />
                    <button
                        onClick={onRemove}
                        type="button"
                        aria-label={`Remove ${label}`}
                        className="absolute top-2 right-2 z-10 bg-red-500 text-white p-2 rounded-full shadow-lg hover:bg-red-600 transition-all md:top-1/2 md:right-1/2 md:translate-x-1/2 md:-translate-y-1/2 md:opacity-0 md:group-hover:opacity-100"
                    >
                        <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
                        </svg>

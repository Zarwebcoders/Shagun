    ...
    return (
        <div className="w-full space-y-8">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h2 className="text-4xl font-bold text-white mb-2">Level Income</h2>
                    <p className="text-[#b0b0b0] text-lg">View your income from network levels</p>
                </div>

                {/* Dropdown for level selection */}
                <div className="flex items-center gap-3">
                    <span className="text-[#b0b0b0]">Select Level:</span>
                    <select
                        value={selectedLevel}
                        onChange={(e) => setSelectedLevel(e.target.value)}
                        className="bg-[#040408] border border-[#9131e7] text-white rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-[#9131e7] min-w-[150px]"
                    >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-gradient-to-br from-[#040408] to-[#1f1f1f] p-6 rounded-lg border border-[#444]">
                    <h3 className="text-[#b0b0b0] text-sm mb-2">Total Level Income</h3>
                    <p className="text-3xl font-bold text-[#9131e7]">${totalLevelIncome.toFixed(2)}</p>
                </div>
                <div className="bg-gradient-to-br from-[#040408] to-[#1f1f1f] p-6 rounded-lg border border-[#444]">
                    <h3 className="text-[#b0b0b0] text-sm mb-2">Total Network Members</h3>
                    <p className="text-3xl font-bold text-blue-400">{totalMembers}</p>
                </div>
                <div className="bg-gradient-to-br from-[#040408] to-[#1f1f1f] p-6 rounded-lg border border-[#444]">
                    <h3 className="text-[#b0b0b0] text-sm mb-2">Active Levels</h3>
            ...
            <div className="space-y-6">
                <h3 className="text-2xl font-bold text-[#9131e7]">
                    ...
                    <table className="w-full">
                        <thead>
                            <tr className="border-b border-[#444]">
                                ...
                                <th className="text-left py-3 px-4 text-white font-semibold">Total Investment</th>
                        ...
                        <tbody>
                            {filteredData.map((item, index) => (
                                <tr key={index} className="border-b border-[#444] hover:bg-[#040408]/50 transition-colors">
                                    ...
                                    <td className="py-3 px-4 text-white">{item.members}</td>
                                    <td className="py-3 px-4 text-white">${item.totalInvestment}</td>
                                    <td className="py-3 px-4 text-white">{item.commissionRate}%</td>
                                    <td className="py-3 px-4 text-[#9131e7] font-bold">${item.income.toFixed(2)}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
